...
def read_and_decode(filename_queue):
    ...
    # label = tf.stack([features['train/label_%s'%x].values for x in ['xmin', 'ymin', 'xmax', 'ymax']])
    # label = tf.transpose(tf.expand_dims(label, 0), [0, 2, 1])
    ...
    images, labels_xmin = tf.train.batch([image, label], batch_size=64, capacity=64, num_threads=8, dynamic_pad=True)
    return images, label
...
with tf.Session() as sess:
    ...
    for i in range(5):
        ...
        print('anno shape:', anno.shape, 'value: ', anno)
